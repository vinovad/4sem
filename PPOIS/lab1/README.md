...
### Как работает: ### 
Класс хранит все данные о продукте и предоставляет методы для их изменения. При создании продукт имеет начальные значения атрибутов, которые не соответствуют стандартам, и начинает с первого этапа производства.
---
...
###Как работает:###
Каждый стандарт определяет конкретные требования к качеству продукта. При проверке соответствия система сравнивает атрибуты продукта с требованиями выбранного стандарта.
---
...
###Как работает:### 
Каждый этап проверяет конкретный атрибут продукта. Чтобы перейти к следующему этапу, продукт должен соответствовать требованиям текущего этапа и иметь действительный сертификат.
---
...
###Как работает:###
При выдаче сертификата устанавливается срок действия (1 год). Система проверяет действительность сертификата перед разрешением перехода к следующему этапу производства.
---
...
###Как работает:###
 Класс хранит информацию об отзыве и проверяет корректность рейтинга (от 1 до 5). Отзывы используются для анализа качества продукта.
---
...
###Как работает:###
Контроллер является центральным элементом системы, связывающим все остальные классы. Он реализует бизнес-логику приложения и обеспечивает взаимодействие между различными компонентами системы.
---
...
Вот обновленный раздел документации с актуальными данными:
...
# Запуск с генерацией отчета о покрытии в HTML
pytest tests.py --cov=domain --cov-report=html
```
